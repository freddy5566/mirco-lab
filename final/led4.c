...
static ssize_t drv_read(struct file *filp, char *buf, size_t count, loff_t *ppos){
	...
	// LED value
	fp = filp_open("/sys/class/gpio/gpio166/value", O_RDONLY, 0);
	if (fp == NULL) {
		printk("166 value cannot be read\n");
		return -1;
	}
	vfs_read(fp, result, strlen(result), &pos);
	filp_close(fp, NULL);
	pos = 0;
	printk("166 value read OK\n");
...
static ssize_t drv_write(struct file *filp, const char *buf, size_t count, loff_t *ppos){
	...
	if(input[0] == '0' || input[0] == '1'){
		...
		if(fp == NULL){
			printk("export 166 can't be set\n");
			return -1;
		}

    
    	vfs_write(fp,"166",strlen("166"),&pos);
		filp_close(fp, NULL);
		pos = 0;
		printk("166 export OK\n");

		fp = filp_open("/sys/class/gpio/gpio166/direction",O_WRONLY,0);
	
		if(fp == NULL){
			printk("dir 166 can't be set\n");
			return -1;
		}
		...
		fp = filp_open("/sys/class/gpio/gpio166/value",O_WRONLY,0);
	
		if(fp == NULL){
			printk("value 166 can't be set\n");
			return -1;
		}
