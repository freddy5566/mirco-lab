...
static ssize_t drv_read(struct file *filp, char *buf, size_t count, loff_t *ppos){
	...
	// LED value
	fp = filp_open("/sys/class/gpio/gpio164/value", O_RDONLY, 0);
	if (fp == NULL) {
		printk("164 value cannot be read\n");
		return -1;
	}
	vfs_read(fp, result, strlen(result), &pos);
	filp_close(fp, NULL);
	pos = 0;
	printk("164 value read OK\n");
...
static ssize_t drv_write(struct file *filp, const char *buf, size_t count, loff_t *ppos){
	...
	if(input[0] == '0' || input[0] == '1'){
		...
		if(fp == NULL){
			printk("export 164 can't be set\n");
			return -1;
		}

    
    	vfs_write(fp,"164",strlen("164"),&pos);
		filp_close(fp, NULL);
		pos = 0;
		printk("164 export OK\n");

		fp = filp_open("/sys/class/gpio/gpio164/direction",O_WRONLY,0);
	
		if(fp == NULL){
			printk("dir 164 can't be set\n");
			return -1;
		}
		...
		fp = filp_open("/sys/class/gpio/gpio164/value",O_WRONLY,0);
	
		if(fp == NULL){
			printk("value 164 can't be set\n");
			return -1;
		}
